...
public class AdjacencyMapGraphTest {
    ...
    public void testOutDegree() {
        ...
        // Verify outgoing edges explicitly using simple assertions
        for (Edge<Integer> edge : undirectedGraph.outgoingEdges(v1))
            assertTrue(edge == e1 || edge == e2 || edge == e3 || edge == e4);

        for (Edge<Integer> edge : undirectedGraph.outgoingEdges(v2))
            assertTrue(edge == e1 || edge == e5 || edge == e6 || edge == e7);

        for (Edge<Integer> edge : undirectedGraph.outgoingEdges(v3))
            assertTrue(edge == e2 || edge == e5 || edge == e8 || edge == e9);

        for (Edge<Integer> edge : undirectedGraph.outgoingEdges(v4))
            assertTrue(edge == e3 || edge == e6 || edge == e8 || edge == e10);

        for (Edge<Integer> edge : undirectedGraph.outgoingEdges(v5))
            assertTrue(edge == e4 || edge == e7 || edge == e9 || edge == e10);
        ...
        // Explicit check for directed edges
        for (Edge<Integer> edge : directedGraph.outgoingEdges(v1))
            assertTrue(edge == e1 || edge == e2 || edge == e3 || edge == e4);

        for (Edge<Integer> edge : directedGraph.outgoingEdges(v2))
            assertTrue(edge == e5 || edge == e6 || edge == e7);

        for (Edge<Integer> edge : directedGraph.outgoingEdges(v3))
            assertTrue(edge == e8 || edge == e9);

        Iterator<Edge<Integer>> itV4 = directedGraph.outgoingEdges(v4).iterator();
        assertTrue(itV4.hasNext());
        assertEquals(e10, itV4.next());
        assertFalse(itV4.hasNext());

        Iterator<Edge<Integer>> itV5 = directedGraph.outgoingEdges(v5).iterator();
        assertTrue(itV5.hasNext());
        assertEquals(e11, itV5.next());
        assertFalse(itV5.hasNext());
    ...
    public void testInDegree() {
        ...
        // Explicitly verify the incoming edges for vertex v1
        int edgeCount = 0;
        for (Edge<Integer> edge : undirectedGraph.incomingEdges(v1)) {
            assertTrue("Unexpected edge for v1", 
                edge == e1 || edge == e2 || edge == e3 || edge == e4);
            edgeCount++;
        }
        assertEquals("v1 should have exactly 4 incoming edges", 4, edgeCount);

        // Explicitly verify the incoming edges for vertex v2
        edgeCount = 0;
        for (Edge<Integer> edge : undirectedGraph.incomingEdges(v2)) {
            assertTrue("Unexpected edge for v2", 
                edge == e1 || edge == e5 || edge == e6 || edge == e7);
            edgeCount++;
        }
        assertEquals("v2 should have exactly 4 incoming edges", 4, edgeCount);

        // Explicitly verify the incoming edges for vertex v3
        edgeCount = 0;
        for (Edge<Integer> edge : undirectedGraph.incomingEdges(v3)) {
            assertTrue("Unexpected edge for v3", 
                edge == e2 || edge == e5 || edge == e8 || edge == e9);
            edgeCount++;
        }
        assertEquals("v3 should have exactly 4 incoming edges", 4, edgeCount);

        // Explicitly verify the incoming edges for vertex v4
        edgeCount = 0;
        for (Edge<Integer> edge : undirectedGraph.incomingEdges(v4)) {
            assertTrue("Unexpected edge for v4", 
                edge == e3 || edge == e6 || edge == e8 || edge == e10);
            edgeCount++;
        }
        assertEquals("v4 should have exactly 4 incoming edges", 4, edgeCount);

        // Explicitly verify the incoming edges for vertex v5
        edgeCount = 0;
        for (Edge<Integer> edge : undirectedGraph.incomingEdges(v5)) {
            assertTrue("Unexpected edge for v5", 
                edge == e4 || edge == e7 || edge == e9 || edge == e10);
            edgeCount++;
        }
        assertEquals("v5 should have exactly 4 incoming edges", 4, edgeCount);

        // v6 should have no incoming edges
        assertFalse("v6 should have no incoming edges",
            undirectedGraph.incomingEdges(v6).iterator().hasNext());
        ...
        // Verify directed inDegree counts explicitly
        ...
        // Verify explicitly for v5 as an example
        edgeCount = 0;
        for (Edge<Integer> edge : directedGraph.incomingEdges(v5)) {
            assertTrue("Unexpected edge for directed v5", 
                edge == e4 || edge == e7 || edge == e9 || edge == e10);
            edgeCount++;
        }
        assertEquals("v5 should have exactly 4 incoming edges", 4, edgeCount);
        ...
        assertFalse("Directed v1 should have no incoming edges",
            directedGraph.incomingEdges(v1).iterator().hasNext());
    }
