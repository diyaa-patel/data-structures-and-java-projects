...
public class SplayTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void splay(Position<Entry<K, V>> p) {
    	...
    	while (!isRoot(p)) {
    	    ...
    	    else if((left(parent) == p && left(grandparent) == parent) || (right(parent) == p && right(grandparent) == parent)) { 
    	      // ZIG-ZIG
     ...
     * parent of p.
     * 
     * @param p the position to check for access 
     */
     ...
     * the tree.
     * 
     * @param p the position to check for insert 
     */
     ...
     * 
     * @param p the position to check for delete 
     */
