...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void rebalance(Position<Entry<K, V>> p) {
    	...
    	do {
    	   oldHeight = getProperty(temp); 
    	   System.out.println("Checking balance at node: " + temp.getElement().getKey() + " with height: " + oldHeight);
    	   if (!isBalanced(temp)) {
    		   // Find the child with the "taller" height
    		   Position<Entry<K, V>> child = tallerChild(temp); 
    	       ...
    	       temp = restructure(grandchild);

    	       recomputeHeight(left(temp)); 
    	       recomputeHeight(right(temp));
    	    } 
    	    recomputeHeight(temp); 
    	    newHeight =  getProperty(temp); 
    	    // Move up to the parent
    	    temp = parent(temp); 
    	} while (oldHeight != newHeight && temp != null); 
     ...
     * 
     * @param node the position to rebalance 
     */
     ...
     * 
     * @param node the position to rebalance if node is not the root 
     */
