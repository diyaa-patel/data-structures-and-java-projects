...
public class MinimumSpanningTreeUtil {
     ...
     * @param g the graph the graph for which to compute a minimum spanning tree
    ...
    public static <V, E extends Weighted> PositionalList<Edge<E>> kruskal(Graph<V, E> g) {
        ...
        HeapPriorityQueue<Integer, Edge<E>> pq = new HeapPriorityQueue<>();
        for (Edge<E> e : g.edges()) {
            pq.insert(e.getElement().getWeight(), e);
        }
   
        UpTreeDisjointSetForest<Vertex<V>> forest = new UpTreeDisjointSetForest<>();
        ...
        while (components > 1 && !pq.isEmpty()) {
            ...
            Position<Vertex<V>> r1 = forest.find(ends[0]);
            Position<Vertex<V>> r2 = forest.find(ends[1]);
            if (r1 != r2) {
                forest.union(r1, r2);
                mst.addLast(edge);
                components--;
